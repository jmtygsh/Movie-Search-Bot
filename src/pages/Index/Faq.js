import React from 'react'

const Faq = () => {
    return (
        <div className='text-center w-4/5 m-auto'>
            <h2 className='text-xl lg:text-2xl font-semibold'>Frequently Asked Questions</h2>
            <div className='mt-4 lg:mt-10' >
                <div className='text-left border-solid border-2 p-5 lg:p-10 rounded-lg'>
                    <h2 className='text-[19px] lg:text-xl mb-2'><i className='bx bx-file'></i>How to use dumb-bots?</h2>
                    <p className='ml-3 text-[15px] leading-6 lg:leading-7'>Are you in the mood for some entertainment? Whether it&apos;s movies, web-series, or anime,
                        finding what you&apos;re looking for is just a few clicks away!
                        Simply type the name of your desired content into the search bar,
                        hit enter, and voila - you&apos;re good to go!
                        <br />Just keep in mind, using too many keywords can be overwhelming, so make sure to use spaces between words
                        for a smoother search experience. With this easy-peasy process,
                        you&apos;ll be enjoying your favorite shows and films in no time - without any hassle!</p>
                </div>

                <div className='text-left border-solid border-2 p-5 lg:p-10 rounded-lg mt-7'>
                    <h2 className='text-[18px] lg:text-xl mb-2'><i className='bx bx-file'></i>What is dumb-bots search?</h2>
                    <p className=' ml-3 text-[15px] leading-6 lg:leading-7'>This is a bot, which bring the data from various of free websites all in one place!
                        And the best part? It&apos;s absolutely free to use! Simply enter your search query and our bot will do the rest,
                        So why waste your time searching multiple websites when you can find everything you need right here?
                        Try it out now and enjoy endless entertainment options at your fingertips!</p>
                </div>

                <div className='text-left border-solid border-2 p-5 lg:p-10 rounded-lg mt-7'>
                    <h2 className='text-[18px] lg:text-xl mb-2'><i className='bx bx-file'></i>How to buy dumb-bots <span className='lg:pl-0 pl-3 md:pl-0'>script?</span></h2>
                    <p className=' ml-3 text-[15px] leading-6 lg:leading-7'>We&apos;re excited to announce that you can now purchase dumb-bots
                        through our pricing page. If you encounter any issues with the &apos;Buy Now&apos;
                        button, please don&apos;t hesitate to contact us at devygsh@gmail.com.
                        We also offer &apos;Fully control&apos;, &apos;Bug Fix Updates&apos;,
                        so, what are you wating for buy it now.</p>
                </div>

                <div className='text-left border-solid border-2 p-5 lg:p-10 rounded-lg mt-7'>
                    <h2 className='text-[18px] lg:text-xl mb-2'><i className='bx bx-file'></i>How to provide contribution <span className='lg:pl-0 pl-3 md:pl-0'>to dumb-bots?</span></h2>
                    <p className=' ml-3 text-[15px] leading-6 lg:leading-7'>If you encounter any issues with dumb-bots search result,
                        please don&apos;t hesitate to contact us through our
                        &apos;Contact Us&apos; page. We value your feedback,
                        so please feel free to share any tips or suggestions
                        for additional features that you would like to
                        see in dumb-bots. <u className='text-red-500'>So let&apos;s make dumb-bots free for all and easy to use together</u>,
                        Thank you for your support.</p>
                </div>

                <div className='text-left border-solid border-2 p-5 lg:p-10 rounded-lg  mt-7 mb-24'>
                    <h2 className='text-[18px] lg:text-xl mb-2'><i className='bx bx-file'></i>How to download from <span className='lg:pl-0 pl-3 md:pl-0'>dumb-bots?</span></h2>
                    <p className=' ml-3 text-[15px] leading-6 lg:leading-7'>Our bots only bring the data from various websites that you are looking for, but the download process is handled by third-party websites beacuse we don&apos;t host or store any files, Our bots works real time.
                    </p>
                </div>


            </div>
        </div>
    )
}

export default Faq
