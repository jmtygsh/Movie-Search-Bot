        ...
        const fetchMovies = async () => {
            ...
            try {
                ...
                setTimeout(async () => {
                    ...
                    await axios.post('https://api.dumb-bots.online/bolly', { keywordValue: name, passcode: "uydhtsdd245@" })
                        .then(res => {
                            if (!res.data.result) {
                            ...
                            <div key={index} className="bg-black text-white rounded-lg ">
                                ...
                                <Link href={item.url} target="_blank" rel="noreferrer">
                                    <h3 className='text-xs md:text-base lg:text-base w-9/12 mt-5 p-2'>{item.url.replace(
                                        /https:\/\/hdmovie2\.hair\/|https:\/\/topmovies\.one\/|https:\/\/vegamovies\.chat\//g, "")
                                        .toUpperCase()}
                                    </h3>
                                </Link>
                            </div>
