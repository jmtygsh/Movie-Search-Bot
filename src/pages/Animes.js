...
const Animes = () => {
        ...
        const fetchMovies = async () => {
            ...
            try {
                ...
                setTimeout(async () => {
                    ...
                    await axios.post('https://api.dumb-bots.online/anime', { keywordValue: name, passcode: "uydhtsdd245@" })
                        .then(res => {
                            if (!res.data.result) {
    ...
    return (
        <div>
            <Head>
                <title>Animes Results - dumb-bots </title>
                <meta name="description" content="Just type the name of the keyword that you are looking for and get all the Animes movies and web series result in one place " />
                            ...
                            <div key={index} className="bg-black text-white rounded-lg ">
                                ...
                                <Link href={item.url} target="_blank" rel="noreferrer">
                                    <h3 className='text-xs md:text-base lg:text-base w-9/12 mt-5 p-2'>{item.url.replace(
                                        /https:\/\/animeflix\.net.in\/|https:\/\/animeheaven\.ru\//g, "")
                                        .toUpperCase()}
                                    </h3>
                                </Link>
                            </div>
